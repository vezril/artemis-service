...
The tag model for Artemis: canonical tag names, five categories, aliases,
transitive implications, and the canonicalization pipeline that runs on the write
path before a `TagsChanged` event is emitted. Requirements here are storage-backend
agnostic; the search read model consumes their output.
