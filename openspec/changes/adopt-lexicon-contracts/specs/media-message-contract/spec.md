# media-message-contract

The Artemis↔Hephaestus async media messages — `ProcessMediaJob` (Artemis → `media.process`),
`MediaProcessed` (← `media.processed`), `MediaFailed` (← `media.failed`) — consumed from the shared
`io.codex:lexicon-messages` types and (de)serialized on the HermesMQ wire as protobuf **canonical
JSON**. Artemis holds no hand-written copy of these shapes; the `codex.messages.v1` case classes come
from Lexicon so a producer/consumer field mismatch is a build error.
